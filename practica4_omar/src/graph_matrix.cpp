...
edi::GraphMatrix::GraphMatrix(const unsigned int& capacity,
...
const float& edi::GraphMatrix::adjacent(const edi::Vertex& u,
                                        const edi::Vertex& v) const {
...
void edi::GraphMatrix::addEdge(const float      & data,
                               const edi::Vertex& u,
                               const edi::Vertex& v) {
  #ifndef NDEBUG
...
void edi::GraphMatrix::beginEdge(const edi::Vertex& u) {
  ...
  for (size_t i = 0; i < _numVertexes and not found; i++) {
    if (i != u.getLabel() and _weights[u.getLabel()][i] !=
...
void edi::GraphMatrix::nextEdge() {
  ...
  for (size_t i = _currEdge->second().getLabel() + 1; i < _numVertexes; i++) {
    if (i !=
        _currEdge->first().getLabel() and _weights[_currEdge->first().getLabel()]
        [i]
        != std::numeric_limits<float>::infinity() and not found) {
      found = true;
